...
#Calcula o indice Gini para uma  divisão de dataset
def gini_index(groups, classes):
	#conta todas as amostras como um ponto de divisão
	...
	for group in groups:
		...
		if size == 0:
			continue
		score = 0.0
		#pontuação do grupo baseado na pontuação de cada classe
...
#seleciona o melhor ponto de divisão para o dataset
...
#cria um valor de nó terminal
def to_terminal(group):
	outcomes = [row[-1] for row in group]
	return max(set(outcomes), key=outcomes.count)

#cria divisões filhas para um nó ou terminal
def split(node, max_depth, min_size, depth):
	left, right = node['groups']
	del(node['groups'])
	#confere para um não divisão
	if not left or not right:
		node['left'] = node['right'] = to_terminal(left+right)
		return
	#confere a profundidade máxima
	if depth >= max_depth:
...
#constroi uma arvore de decisão
def build_tree(train, max_depth, min_size):
	root = get_split(train)
	split(root, max_depth, min_size, 1)
	return root

#Faz uma predição com a arvore de decisão
...
#Classificação e algoritmo de arvore de regressão
